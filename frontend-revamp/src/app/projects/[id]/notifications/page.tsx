...
import { NotificationAPI, ProjectsAPI } from '@/lib/api';
import { 
  FiSend, 
  FiMail, 
  FiSlack, 
...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    const checkPermission = async () => {
      try {
        // Fetch project members untuk memeriksa peran pengguna saat ini
        ...
        if (token) {
          try {
            ...
            // Cari pengguna saat ini di antara anggota proyek
            const currentUser = members.find(member => member.user.id === userId);
            
            if (currentUser) {
              const role = currentUser.role;
              
              // Verifikasi apakah pengguna adalah admin atau pemilik
              if (role === 'admin' || role === 'owner') {
                setHasPermission(true);
              } else {
                setHasPermission(false);
                setError('Hanya admin atau pemilik proyek yang dapat mengakses pengaturan notifikasi.');
              }
            } else {
              setHasPermission(false);
              setError('Anda tidak terdaftar sebagai anggota proyek ini.');
            }
          } catch (error) {
            console.error('Error parsing JWT token:', error);
            setHasPermission(false);
            setError('Terjadi kesalahan saat memverifikasi izin Anda.');
          }
        } else {
          setHasPermission(false);
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        const completeSettings = {
          ...
          slackChannel: '', // Default nilai
          slackUsername: 'LogRaven Bot', // Default nilai
          slackEmoji: ':robot:', // Default nilai
          whatsapp: data.whatsapp || false,
          whatsappNumber: data.whatsappNumber || '',
          whatsappVerified: data.whatsappVerified || false,
          notifyOnNewError: data.notifyOnNewError !== undefined ? data.notifyOnNewError : true,
          notifyOnResolvedError: data.notifyOnResolvedError || false,
          minimumErrorLevel: data.minimumErrorLevel || 'error'
        };
        ...
        if (err && typeof err === 'object' && 'status' in err && err.status === 403) {
          setError('Anda tidak memiliki izin untuk melihat pengaturan notifikasi.');
        } else {
    ...
    if (hasPermission) {
      ...
      setLoading(false); // Jangan loading terus jika tidak ada permission
    }
  }, [projectId, hasPermission]);

  const handleSaveSettings = async () => {
    if (!hasPermission) {
      toast.error('Anda tidak memiliki izin untuk mengubah pengaturan notifikasi');
      return;
    }
  ...
  const handleTestNotification = async (type: 'email' | 'slack' | 'whatsapp') => {
    if (!hasPermission) {
      toast.error('Anda tidak memiliki izin untuk menguji notifikasi');
      return;
    }
      ...
      <div className="space-y-6">
        <div className="flex flex-col space-y-2">
          ...
          <p className="text-muted-foreground">
            Konfigurasi bagaimana dan kapan Anda menerima notifikasi
          </p>
        </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Diperlukan Hak Akses Admin
              </CardTitle>
              <CardDescription>
                Hanya admin atau pemilik proyek yang dapat mengelola pengaturan notifikasi
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex flex-col items-center justify-center py-8">
                <FiLock className="h-16 w-16 text-muted-foreground mb-4" />
                <p className="text-lg font-medium text-center">Akses Terbatas</p>
                <p className="text-muted-foreground mt-2 text-center max-w-md">
                  Untuk keamanan, pengaturan notifikasi hanya dapat diakses oleh admin atau pemilik proyek.
                  Hubungi admin proyek jika Anda memerlukan bantuan dengan pengaturan notifikasi.
                </p>
              </div>
            </CardContent>
          </Card>
                  ...
                  {settings.email && (
                    <CardFooter className="bg-blue-50/30 border-t flex justify-end py-3">
                      <Button 
                        ...
                        disabled={isTestingEmail || !hasPermission}
                        className="gap-2"
                      >
                        {isTestingEmail ? (
                          <>
                  ...
                  {settings.slack && (
                    <CardFooter className="bg-purple-50/30 border-t flex justify-end py-3">
                      <Button 
                        ...
                        disabled={isTestingSlack || !settings.slackWebhookUrl || !hasPermission}
                        className="gap-2"
                      >
                        {isTestingSlack ? (
                          <>
                        ...
                        <div>
                          ...
                          <div className="flex space-x-2">
                            <Input 
                              ...
                              className="text-sm"
                              disabled={settings.whatsappVerified || !hasPermission}
                            />
                            {!settings.whatsappVerified && (
                              <Button 
                                variant="outline" 
                                ...
                                disabled={isVerifyingWhatsapp || !settings.whatsappNumber || !hasPermission}
                              >
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="flex space-x-2">
                                  ...
                                  <Button 
                                    ...
                                    disabled={isVerifyingWhatsapp || !verificationCode || !hasPermission}
                                  >
                                ...
                                <p className="text-xs text-amber-700">
                                  Kode tidak diterima? <button 
                                    type="button" 
                                    className={`text-amber-800 underline ${!hasPermission ? 'opacity-50 cursor-not-allowed' : ''}`}
                                    onClick={handleVerifyWhatsapp}
                                    disabled={isVerifyingWhatsapp || !hasPermission}
                                  >
                                    Kirim ulang kode
                                  </button>
                                </p>
                  ...
                  {settings.whatsapp && settings.whatsappVerified && (
                    <CardFooter className="bg-green-50/30 border-t flex justify-end py-3">
                      <Button 
                        ...
                        disabled={isTestingWhatsapp || !settings.whatsappVerified || !hasPermission}
                        className="gap-2"
                      >
                        {isTestingWhatsapp ? (
                          <>
                      ...
                      <div>
                        ...
                        >
                          <div className="flex items-center space-x-3 p-2 rounded hover:bg-accent/20 transition-colors">
                            <RadioGroupItem value="info" id="error-level-info" disabled={!hasPermission} />
                          ...
                          <div className="flex items-center space-x-3 p-2 rounded hover:bg-accent/20 transition-colors">
                            <RadioGroupItem value="warning" id="error-level-warning" disabled={!hasPermission} />
                            <div className="flex items-center space-x-2">
                          ...
                          <div className="flex items-center space-x-3 p-2 rounded hover:bg-accent/20 transition-colors">
                            <RadioGroupItem value="error" id="error-level-error" disabled={!hasPermission} />
                            <div className="flex items-center space-x-2">
                          ...
                          <div className="flex items-center space-x-3 p-2 rounded hover:bg-accent/20 transition-colors">
                            <RadioGroupItem value="fatal" id="error-level-fatal" disabled={!hasPermission} />
                            <div className="flex items-center space-x-2">
